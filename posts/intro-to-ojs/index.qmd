...
`convert_time_hours` turns a time string like `12:34` into the number of seconds. 
...
`query_endpoint` is a helper function to query the N.H.L. api. It takes an endpoint and a list of query parameters and returns a tibble with the results. It is always helpful to isolate the code that does the querying from the code that does the data wrangling.
